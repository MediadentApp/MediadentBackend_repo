...
import { loginServiceErrorResponse, otherLoginServiceErrorResponse, } from '../services/auth/checkUserExists.service.js';
import ApiError from '../utils/ApiError.js';
import ApiResponse from '../utils/ApiResponse.js';
import { createSendToken, sendDeleteToken } from '../utils/authUtils.js';
import catchAsync from '../utils/catchAsync.js';
import { generateOTP } from '../utils/index.js';
import crypto from 'crypto';
import { sendMail } from '../libs/sendMail.js';
import { formatErrorMessageTemplate } from '../utils/dataManipulation.js';
export const emailReg = catchAsync(async (req, res, next) => {
    ...
    if (tempUserDb) {
        if (tempUserDb.emailVerified) {
            return next(new ApiError(responseMessages.AUTH.EMAIL_ALREADY_VERIFIED, 200, ErrorCodes.SIGNUP.EMAIL_ALREADY_VERIFIED));
        }
        if (tempUserDb.otpSendAt && (await tempUserDb.checkOtpTime())) {
            const timeLeftInSeconds = Math.ceil((tempUserDb.otpSendAt.getTime() + appConfig.otp.sendOtpAfter * 1000 - Date.now()) / 1000);
            const timeLeft = `${timeLeftInSeconds} seconds`;
            return next(new ApiError(formatErrorMessageTemplate(responseMessages.AUTH.OTP_ALREADY_SENT, {
                time: timeLeft,
            }), 400, ErrorCodes.SIGNUP.OTP_ALREADY_SENT));
        }
...
export const emailVerify = catchAsync(async (req, res, next) => {
    ...
    if (tempUser.emailVerified)
        return next(new ApiError(responseMessages.AUTH.EMAIL_ALREADY_VERIFIED, 409, ErrorCodes.SIGNUP.EMAIL_ALREADY_VERIFIED));
...
export const signupDetails = catchAsync(async (req, res, next) => {
...
// Shouldn't be used with Protect middleware
export const signupInterests = catchAsync(async (req, res, next) => {
...
export const login = catchAsync(async (req, res, next) => {
    ...
    if (!email || !password) {
        return next(new ApiError(responseMessages.AUTH.INCOMPLETE_CREDENTIALS, 400, ErrorCodes.LOGIN.INCOMPLETE_CREDENTIALS));
    }
...
export const resetPassword = catchAsync(async (req, res, next) => {
    ...
    if (!password || !passwordConfirm) {
        return next(new ApiError(responseMessages.CLIENT.MISSING_INVALID_INPUT, 400, ErrorCodes.PASSWORD_RESET.PROVIDE_NEW_PASSWORD));
    }
...
export const updatePassword = catchAsync(async (req, res, next) => {
    ...
    if (!currentPassword || !updatedPassword || !updatedPasswordConfirm) {
        return next(new ApiError(responseMessages.AUTH.INCORRECT_CREDENTIALS, 400, ErrorCodes.PASSWORD_UPDATE.INCOMPLETE_CREDENTIALS));
    }
    ...
    if (!isPasswordCorrect) {
        return next(new ApiError(responseMessages.AUTH.INCORRECT_PASSWORD, 401, ErrorCodes.PASSWORD_UPDATE.INCORRECT_PASSWORD));
    }
