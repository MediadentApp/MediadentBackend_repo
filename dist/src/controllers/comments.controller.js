...
export const createComment = catchAsync(async (req, res, next) => {
    ...
    if (!postId || !content?.trim()) {
        return next(new ApiError(responseMessages.CLIENT.MISSING_ALL_NECESSARY_REQUEST_DATA, 400, ErrorCodes.CLIENT.MISSING_INVALID_INPUT));
    }
...
export const getComments = catchAsync(async (req, res, next) => {
    const { postId, commentId, limit = '5', // Limit for the top-level comments
    skip = '0', // Skip for pagination of the top-level comments
    page, children = '1', // Depth of children (1 = one level, 2 = two levels, etc.)
    ...
    const commentAggregation = [
        ...
        ...(postId
            ? [
                {
                    $addFields: sortMethod === SortMethod.Votes
    ...
    const buildTree = (comment, childrenMap) => {
        ...
        const sortedChildren = paginatedChildren.sort((a, b) => {
    ...
    return ApiResponse(res, 200, responseMessages.GENERAL.SUCCESS, commentId ? structured[0] : structured, extra);
});
/**
...
export const voteComment = catchAsync(async (req, res, next) => {
    ...
    if (!existingVote) {
        ...
        updateOp = Comment.updateOne({ _id: commentId }, { $inc: voteType === VoteEnum.upVote ? { upvotesCount: 1 } : { downvotesCount: 1 } });
    }
        ...
        updateOp = Comment.updateOne({ _id: commentId }, voteType === VoteEnum.upVote
