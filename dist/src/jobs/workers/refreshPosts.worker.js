...
export const refreshPostsWorker = new Worker('refreshPosts', async (job) => {
...
refreshPostsWorker.on('completed', job => {
    console.log(`Job ${job.id} completed. Updated: ${job.returnvalue.updated}, Dropped PostViews: ${job.returnvalue.droppedPostViews}`);
});
