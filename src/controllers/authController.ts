...
import { createSendToken } from '@src/utils/authUtils';
...
export const emailReg = catchAsync(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    const otpExpiration = new Date(
      Date.now() + config.otp.otpExpiration * 60 * 1000,
    );
...
export const signupInterests = catchAsync(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (interests.length < config.app.numOfSignupInterests) {
      return next(
        new AppError(
          `Choose at least ${config.app.numOfSignupInterests} interests`,
          406,
        ),
      );
    }
...
export const fetchUser = catchAsync(async (req: Request, res: Response) => {
  const { user } = req as Request & { user: typeof User };
