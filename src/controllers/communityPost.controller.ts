 ...
 */
export const updateCommunityAvatar = catchAsync(
  async (req: AppRequest<IdParam>, res: AppResponse, next: NextFunction) => {
    ...
    if (!file) {
      return next(
        new ApiError(responseMessages.APP.COMMUNITY.IMAGE_REQUIRED, 400, ErrorCodes.CLIENT.MISSING_INVALID_INPUT)
      );
    }
    ...
    if (oldAvatarUrl) void deleteImagesFromS3([oldAvatarUrl]);
 ...
 */
export const updateCommunityBanner = catchAsync(
  async (req: AppRequest<IdParam>, res: AppResponse, next: NextFunction) => {
    const { id: communityId } = req.params;
    const file = (req.files as { banner?: Express.Multer.File[] })?.banner?.[0];

    if (!file) {
      return next(
        new ApiError(responseMessages.APP.COMMUNITY.BANNER_REQUIRED, 400, ErrorCodes.CLIENT.MISSING_INVALID_INPUT)
      );
    }
    ...
    // 🧹 Delete old image if it existed (non-blocking)
    if (oldBannerUrl) void deleteImagesFromS3([oldBannerUrl]);
 ...
 */
export const getSavedPosts = catchAsync(
 ...
 */
export const getUpvotedPosts = catchAsync(
  async (req: AppPaginatedRequest, res: AppPaginatedResponse, next: NextFunction) => {
    const userId = req.user._id;
 ...
 */
export const getDownvotedPosts = catchAsync(
  async (req: AppPaginatedRequest, res: AppPaginatedResponse, next: NextFunction) => {
    const userId = req.user._id;
