import { College, CityStates } from '@src/models';
...
import {
  ...
  University,
  CityStates,
} from '@src/models/userFormatModel';
import User from '@src/models/userModel';
import AppError from '@src/utils/appError';
import catchAsync from '@src/utils/catchAsync';
import { findKeyValues, stringToObjectID } from '@src/utils/util';
...
// Fetch user types
export const userTypes = catchAsync(
  async (req: Request, res: Response, next: NextFunction) => {
    const userTypes = await UserFormat.findOne({}, 'userType');
    if (!userTypes || !userTypes.userType)
      return next(new AppError('Could not find User Type Options', 404));
...
// Fetch user genders
export const userGenders = catchAsync(
  async (req: Request, res: Response, next: NextFunction) => {
    const userGenders = await UserFormat.findOne({}, 'userGender');
    if (!userGenders || !userGenders.userGender)
      return next(new AppError('Could not find Gender Options', 404));
...
export const userInterests = catchAsync(
  async (req: Request, res: Response, next: NextFunction) => {
    const userInterests = await UserFormat.findOne({}, 'userInterest');
    if (!userInterests || !userInterests.userInterest)
      return next(new AppError('Could not find Interest Options', 404));

    res.status(200).json({
      data: {
        interests: userInterests.userInterest,
        required: config.app.numOfSignupInterests,
      },
    });
  },
);
...
export const allStreams = catchAsync(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!streams || streams.length === 0)
      return next(new AppError('Could not find streams list', 404));
...
export const allGraduationCourses = catchAsync(
  async (req: Request, res: Response, next: NextFunction) => {
    const result = await UserFormat.findOne(
      {},
      'userAcademicDetails.graduation',
    );
    const graduationCourses = result?.userAcademicDetails?.graduation;
    if (!graduationCourses)
      return next(new AppError('Could not find graduation courses list', 404));
...
export const allPostGraduationCourses = catchAsync(
  async (req: Request, res: Response, next: NextFunction) => {
    const result = await UserFormat.findOne(
      {},
      'userAcademicDetails.postGraduation',
    );
    const postGraduationCourses = result?.userAcademicDetails?.postGraduation;
    if (!postGraduationCourses) {
      return next(
        new AppError('Could not find post-graduation courses list', 404),
      );
    }
...
export const allProfessions = catchAsync(
  async (req: Request, res: Response, next: NextFunction) => {
    const result = await UserFormat.findOne(
      {},
      'userAcademicDetails.professions',
    );
    const professions = result?.userAcademicDetails?.professions;
    if (!professions || professions.length === 0)
      return next(new AppError('Could not find professions list', 404));
...
export const getAllUniversities = catchAsync(
  async (req: Request, res: Response, next: NextFunction) => {
...
export const searchCollege = catchAsync(
  async (req: Request, res: Response, next: NextFunction) => {
    const keyword = req.query.keyword as string;

    if (!keyword) {
      return next(
        new AppError('Keyword is required for searching colleges', 400),
      );
    }
    ...
    if (colleges.length === 0) {
      return next(
        new AppError('No colleges found matching the search criteria', 404),
      );
    }

    const formattedColleges = colleges.map(
      (college) => `${college.name} - ${college.state}, ${college.city}`,
    );
...
export const getCityStates = catchAsync(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    const paginatedCitiesStatesArray =
      citiesStatesData[0]?.paginatedCitiesStatesArray || [];
...
export const getByState = catchAsync(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (result.length === 0) {
      return next(
        new AppError('No colleges found for the specified state', 404),
      );
    }
...
export const getByDistrict = catchAsync(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (result.length === 0) {
      return next(
        new AppError('No colleges found for the specified district', 404),
      );
    }
...
export const getDistrictsByState = catchAsync(
  async (req: Request, res: Response, next: NextFunction) => {
    const state = (req.query.state as string)?.toLowerCase();
    if (!state) {
      return next(new AppError('State is required', 400));
    ...
    if (districts.length === 0) {
      return next(
        new AppError('No districts found for the specified state', 404),
      );
    }
